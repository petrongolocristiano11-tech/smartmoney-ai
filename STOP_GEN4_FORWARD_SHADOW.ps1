param(
    [string]$CampaignId = "",
    [string]$Note = "Manual forward campaign stop"
)
...
$output = Join-Path $HOME "Downloads\smartmoney-gen4-forward-stop-$timestamp.json"
...
try {
    $args = @(
        ...
        "stop",
        "--confirmation", "STOP_GEN4_STRICT_FORWARD_SHADOW",
    ...
    & $python @args
    if ($LASTEXITCODE -ne 0) { throw "Chiusura campagna Gen4 forward non riuscita." }
} finally {
    if ($null -eq $previous) {
