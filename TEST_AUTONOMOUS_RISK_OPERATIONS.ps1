...
Write-Host "`n[1/7] Compilazione backend, migrazione e test..." -ForegroundColor Cyan
...
Write-Host "`n[3/7] Verifica e applicazione migrazione Alembic..." -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { throw "verifica milestone fallita" }
...
Write-Host "`n[6/7] ESLint dei file modificati e build frontend..." -ForegroundColor Cyan
...
try {
    ...
    npx eslint `
        ...
        src/components/liveTrading/LiveTradingPolicyForm.jsx `
        src/pages/LiveTrading.jsx `
        src/services/liveTradingApi.js
    if ($LASTEXITCODE -ne 0) { throw "ESLint dei file modificati fallito" }
...
Write-Host "`nTUTTI I CONTROLLI DEL MILESTONE SONO TERMINATI CORRETTAMENTE." -ForegroundColor Green
Write-Host "Mantieni RUN_LIVE_POSITION_MONITOR=false finche non hai verificato un ciclo manuale DRY_RUN." -ForegroundColor Yellow
