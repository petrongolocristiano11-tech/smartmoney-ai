...
def _provisional_rejection(
    ...
    *,
    ...
    if quote_latency_ms > int(campaign.max_quote_latency_ms):
        return "QUOTE_TOO_SLOW"
    if out_amount <= 0:
        return "NO_EXECUTABLE_OUTPUT"
    if price_impact_bps > float(campaign.max_price_impact_bps):
        return "PRICE_IMPACT_TOO_HIGH"
    if (
        deterioration_bps is not None
        and deterioration_bps > float(campaign.max_price_deterioration_bps)
    ):
...
def get_gen4_fastpath_shadow_status(db: Session, *, recent_limit: int = 50) -> dict[str, Any]:
    limit = max(1, min(int(recent_limit), 500))
    rows = list(
        db.scalars(
            select(CanonicalParserGen4FastpathShadowEvent)
            .order_by(CanonicalParserGen4FastpathShadowEvent.fast_received_at.desc())
            .limit(limit)
        )
    )
    all_rows = list(db.scalars(select(CanonicalParserGen4FastpathShadowEvent)))
    ...
    leads = [float(row.fast_lead_vs_webhook_ms) for row in reconciled if row.fast_lead_vs_webhook_ms is not None]
    fast_end = [float(row.fast_end_to_quote_ms) for row in reconciled if row.fast_end_to_quote_ms is not None]
    confirmed_end = [
    ...
    return {
        "version": FASTPATH_VERSION,
        "enabled": bool(getattr(settings, "CANONICAL_PARSER_GEN4_FASTPATH_SHADOW_ENABLED", False)),
        "commitment": FASTPATH_COMMITMENT,
        ...
        "fast_provisional_copyable_count": sum(bool(row.fast_provisional_copyable) for row in buys),
        "fast_reconciled_copyable_count": sum(row.fast_reconciled_copyable is True for row in buys),
        "fast_lead_vs_webhook_ms": {"p50": _percentile(leads, 0.50), "p95": _percentile(leads, 0.95)},
        "fast_end_to_quote_ms": {"p50": _percentile(fast_end, 0.50), "p95": _percentile(fast_end, 0.95)},
        "confirmed_path_end_to_quote_ms": {"p50": _percentile(confirmed_end, 0.50), "p95": _percentile(confirmed_end, 0.95)},
        "recent": [
            {
                ...
                "confirmed_path_end_to_quote_ms": row.confirmed_path_end_to_quote_ms,
