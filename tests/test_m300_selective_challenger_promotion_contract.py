...
def test_r12_selected_formal_m74_admission_uses_fresh_m300_gate():
    anchor = datetime(2026, 9, 10, 16, 0, tzinfo=timezone.utc)
    for label, wallet in R12_FORMAL_M74_WALLETS.items():
        assert TARGETS[label] == wallet
        out = evaluate_candidate_promotion(
            wallet=wallet,
            events=_dataset(wallet, anchor, prefix=f"R12-{label}"),
            anchor_utc=anchor,
            terminal_at=anchor + timedelta(hours=2),
        )
        assert out["promotion_eligible"] is True
        assert out["target_admission"]["kind"] == R12_FORMAL_M74_ADMISSION_KIND
        assert out["target_admission"]["upstream_formal_m74_pass"] is True
        assert out["target_admission"]["upstream_formal_m74_report_sha256"] == R12_STATE_SHA256
        assert out["target_admission"]["upstream_admission_readiness_report_sha256"] == R12_FULL31_REPORT_SHA256
        assert out["target_admission"]["candidate_entry_evidence_backfilled"] is False
        assert out["formal_claims"]["m74_pass_claimed"] is False
        assert out["formal_claims"]["m298_pass_claimed"] is False
...
def test_r13_independent_formal_wallets_use_fresh_m300_gate_and_exact_provenance():
    anchor = datetime(2026, 9, 11, 10, 30, tzinfo=timezone.utc)
    for label, wallet in R13_FORMAL_M74_WALLETS.items():
        assert TARGETS[label] == wallet
        out = evaluate_candidate_promotion(
            wallet=wallet,
            events=_dataset(wallet, anchor, prefix=f"R13-{label}"),
            anchor_utc=anchor,
            terminal_at=anchor + timedelta(hours=2),
        )
        assert out["promotion_eligible"] is True
        assert out["target_admission"]["kind"] == R13_FORMAL_M74_ADMISSION_KIND
        assert out["target_admission"]["upstream_formal_m74_pass"] is True
