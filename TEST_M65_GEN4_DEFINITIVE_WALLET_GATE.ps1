...
Write-Host "[2/6] Verifier M58-M65"
$verifiers = @(
...
& $python -m pytest `
    ...
    tests/test_m65_gen4_definitive_wallet_gate.py `
    -q
if ($LASTEXITCODE -ne 0) { throw "Test mirati M58-M65 falliti." }

Write-Host "[4/6] Alembic head immutata"
$headOutput = @(& $python -m alembic heads)
$headExitCode = $LASTEXITCODE
$headOutput | ForEach-Object { Write-Host $_ }
if ($headExitCode -ne 0) { throw "Controllo Alembic heads fallito." }
...
Write-Host "ALEMBIC_MIGRATION=NOT_REQUIRED"
Write-Host "OFFICIAL_REALTIME_TRADES=83"
...
Write-Host "SIGNER_AUTHORIZED=NO"
Write-Host "AUTOMATIC_LIVE_ACTIVATION=NO"
Write-Host "M65_TESTS=PASS" -ForegroundColor Green
